...
# df['Mortgage'] = np.where(df['mortgage_credits_2021']>10000,"Yes","No")
...
# for i in range(bins):
#     while row>=row_num:
#         if ((df.loc[row,'total_income'] >= min_x+(i)*bin_width) and
#             (df.loc[row,'total_income'] <  min_x+(i+1)*bin_width)):
...
for year in range(2021,2027):
    ...
    for i in range(bins):       
        if ((len(df[df['bin_mortgage']==i])) > 0 and
            ...
            # year
            mortgage_rand = random.choices(df[(df['bin_mortgage']==i)&(df['Mortgage_bool_'+str(year)]==1)]['mortgage_credits_'+str(year)].to_list(), k=num_choose)
            ...
            df.loc[index_rand,'mortgage_credits_'+str(year+1)] = mortgage_rand
